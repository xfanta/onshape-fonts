...
function mergeGlyph(glyph: GlyphCurves, paper: Paper): GlyphCurves {
  ...
  try {
    // resolveCrossings + reorient is the most forgiving combination:
    // it removes some overlaps and preserves outer/hole nesting for
    // letters with counters (o, p, q, b). It does not catch tangent
    // contacts (the e crossbar, h serifs) — those edge cases are
    // documented in the UI as "reduces overlaps" rather than promised
    // as a full union. pairwise unite() merged more aggressively but
    // also destroyed inner holes.
